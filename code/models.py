...
class RobertaSpanPredictor(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None):
        ...
        outputs = hidden_states.reshape(-1, hidden_states.size()[-1])
...
class ESTER_Model(nn.Module):
    def __init__(self, basic_model, args):
        super(ESTER_Model, self).__init__()
        self.model = basic_model
        self.args = args
        self.event_projecter = nn.Linear(1024, 1024)
        self.type_classifer = nn.Linear(1024, 5)
