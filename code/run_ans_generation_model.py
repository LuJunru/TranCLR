...
# Copyright (c) 2022, University of Warwick, Department of Computer Science. (authors: Junru Lu)
#
...
type_dict = {"Coreference": 0,
             "Sub-event": 1,
             "Counterfactual Conditional": 2,
             "Causal": 3,
             "Indicative Conditional": 4}
...
def main():
    ...
    if args.do_train:
        ...
        all_type_labels = select_field(train_features, 'types')
        all_type_labels = [[type_dict[type_label]] for type_label in all_type_labels]
        all_type_ids = torch.tensor(all_type_labels, dtype=torch.long)
        ...
        logger.info("id_size: {}, mask_size: {}, instance_key_size: {}, label_size: {}, type_label_size: {}, event_id_size: {}".format(
            all_input_ids.size(), all_input_mask.size(), all_key_indices.size(), all_output_ids.size(), all_type_ids.size(), event_ids_.size()))

        train_data = TensorDataset(all_input_ids, all_input_mask, all_key_indices, all_output_ids, all_output_mask, all_type_ids, 
                                   event_ids_, clr_ids_, all_q_start, all_q_end)
        ...
        global_step = 0
        best_eval_f1 = 0.0
        type_loss_fct = CrossEntropyLoss(ignore_index=-100).to(device)
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            tr_loss, tr_acc_start, tr_acc_end = 0.0, 0.0, 0.0
            nb_tr_examples, nb_tr_steps = 0, 0
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                batch = tuple(t.to(device) for t in batch)
                input_ids, input_masks, instance_indices, output_ids, output_masks, type_ids, event_ids, clr_ids, q_start, q_end = batch
                if "transE" not in args.output_dir:
                    outputs = model(input_ids, attention_mask=input_masks, labels=output_ids,  
                                    decoder_attention_mask=output_masks)
                    loss = outputs["loss"]
                    final_hidden_state = outputs["encoder_last_hidden_state"]
                else:
                    outputs = model.module.model(input_ids, attention_mask=input_masks, labels=output_ids,  
                                                 decoder_attention_mask=output_masks)
                    loss = outputs["loss"]
                    final_hidden_state = outputs["encoder_last_hidden_state"]

                    # TC
                    avg_event_rep = torch.ones((final_hidden_state.size(0), final_hidden_state.size(-1)), device=final_hidden_state.device)  # (batch, 1024)
                    for b_i in range(avg_event_rep.size(0)):
                        if n_gpu > 1:
                            avg_event_rep[b_i, :] = model.module.event_projecter(final_hidden_state[b_i:b_i + 1, q_start[b_i]:q_end[b_i], :]).mean(dim=1)  # (batch, 1024)
                        else:
                            avg_event_rep[b_i, :] = model.event_projecter(final_hidden_state[b_i:b_i + 1, q_start[b_i]:q_end[b_i], :]).mean(dim=1)  # (batch, 1024)
                    if n_gpu > 1:
                        type_logits = model.module.type_classifer(avg_event_rep)
                    else:
                        type_logits = model.type_classifer(avg_event_rep)
                    type_loss = type_loss_fct(type_logits.view(-1, type_logits.size(-1)), type_ids.view(-1))

                    # CL
                    event_rep = torch.gather(final_hidden_state, 1, torch.cat([event_ids.unsqueeze(-1)] * final_hidden_state.size(-1), dim=-1))
                    if n_gpu > 1:
                        event_rep = model.module.event_projecter(event_rep)
                    else:
                        event_rep = model.event_projecter(event_rep)
                    event_rep = torch.nn.functional.normalize(event_rep, dim=-1)  # (batch, 36, 1024)
                    similarity = torch.nn.functional.cosine_similarity(event_rep.unsqueeze(1), event_rep.unsqueeze(2), dim=-1)  # (batch, 36, 36)
                    temperature = 1.0
                    similarity_matix = torch.exp(similarity / temperature)  # (batch, 36, 36)
                    event_clr_loss = []
                    for batch_id in range(input_ids.size(0)):
                        ...
                        batch_event_ids: 6, 10, 15, 22, 81, 391, 391
                        """
                        for event_index in range(valid_batch_clr_len):  # 0 ~ ?
                            if batch_clr_ids[event_index] > 0.5:
                                denominator = similarity_matix[batch_id][event_index][:valid_batch_clr_len].sum()
                                denominator -= similarity_matix[batch_id][event_index][event_index]
                                for clr_index in range(valid_batch_clr_len):  # 0 ~ ?
                                    if clr_index != event_index and batch_clr_ids[clr_index] > 0.5:
                                        numerator = similarity_matix[batch_id][event_index][clr_index]
                                        cur_clr_loss = -torch.log(numerator / denominator)

                                        # weaken a & a event positive pairs
                                        # if batch_clr_ids[event_index] == batch_clr_ids[clr_index]:
                                        #     cur_clr_loss *= 0.5
                ...
                if n_gpu > 1:
                    ...
                    if "transE" in args.output_dir:
                        event_clr_loss = event_clr_loss.mean()
                        type_loss = type_loss.mean()
                if "transE" in args.output_dir:
                    loss += 0.1 * event_clr_loss
                    loss += 0.1 * type_loss
                if args.gradient_accumulation_steps > 1:
            ...
            if args.do_eval:
                ...
                eval_features = convert_to_features(eval_data, tokenizer,
                                                    max_length=args.max_seq_length, evaluation=True, output_dir=args.output_dir, sep_tok=";")
                ...
                eval_type_labels = [[type_dict[type_label]] for type_label in eval_type_labels]
                eval_type_ids = torch.tensor(eval_type_labels, dtype=torch.long)
                ...
                eval_data = TensorDataset(eval_input_ids, eval_input_mask, eval_key_indices, eval_output_ids, eval_type_ids)
                ...
                for batch in tqdm(eval_dataloader, desc="Evaluating"):
                    batch = tuple(t.to(device) for t in batch)
                    input_ids, input_masks, instance_indices, output_ids, type_ids = batch

                    with torch.no_grad():
                        if "transE" in args.output_dir:
                            res = model.module.model.generate(input_ids, attention_mask=input_masks)
                        else:
