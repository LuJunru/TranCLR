...
def main():
    ...
    if args.do_train:
        ...
        logger.info("id_size: {} mask_size: {}, instance_key_size: {}, segment_size: {}, type_label_size: {}, event_id_size: {}".format(
            all_input_ids.size(), all_input_mask.size(), all_key_indices.size(), all_segment_ids.size(), all_type_ids.size(), event_ids_.size()))

        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_key_indices, all_type_ids, 
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_masks, segment_ids, instance_indices, type_ids, event_ids, clr_ids, q_start, q_end = batch
                
                if "transE" not in args.output_dir:
                    logits, final_hidden_state = model.model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids)
                else:
                    logits, final_hidden_state = model.module.model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids)
            ...
            if args.do_eval:
                ...
                eval_features = convert_to_features(eval_data, tokenizer,
                                                    max_length=args.max_seq_length, evaluation=True, output_dir=args.output_dir)
                ...
                for batch in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        if "transE" in args.output_dir and n_gpu > 1:
                            eval_logits, _ = model.module.model(input_ids, attention_mask=input_masks, token_type_ids=segment_ids)
                        else:
